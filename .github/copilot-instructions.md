...
- Use `TextHandler` for streamed output; always call `.reset()` for a fresh run and `.write(data, last)` for updates.- **Logging:** Always use `getLogger(component)` from `src/extension/utils/logging.ts` for diagnostics. Never use `console.*` methods. Log metadata (file paths, command args, error reasons) but avoid logging large payloads or stdin/stdout content.
...
- **Runnable lifecycle:** `Runnable.run()` resets internal state automatically. Always call `.on()` to attach listeners (fluent chaining pattern). Await `.done` to get `RunTermination` value. Call `.dispose()` for cleanup (removes all listeners via `removeAllListeners()`).- **View persistence:** Judge and Stress are designed to preserve in-memory state (and any running `Runnable` processes) when their webviews are hidden/revealed. Lifecycle teardown belongs in `onDispose()`; hiding a view should not implicitly stop runs. Runs should stop/reset when the active editor switches to a different file (file-scoped state), using the same “webview focus may temporarily clear active editor” guard as Judge.
...
- Debugging uses a per-session dynamic port:
  - The extension generates a fresh `${debugPort}` for every debug run and injects it into both `debugCommand` and the selected `launch.json` configuration.
  ...
  - The debug attach flow waits for the chosen port to become “listening” (timeout) before attaching; see `findAvailablePort()` / `waitForPortListening()` and `JudgeViewProvider`’s debug path.
